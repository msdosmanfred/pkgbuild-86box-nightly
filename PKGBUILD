...
build() {
  LDFLAGS='-z now -z shstk' cmake -Bbuild --preset development -DCMAKE_INSTALL_PREFIX=/usr -DQT=on -DDYNAREC=on -DDEV_BRANCH=on -D "BUILD_TYPE=alpha" -D "EMU_BUILD=build ${pkgver}" -D "EMU_BUILD_NUM=$pkgver"
  cmake --build build
}
