...
build() {
  LDFLAGS='-z now -z shstk' cmake -B build --preset regular -D CMAKE_INSTALL_PREFIX=/usr -D NEW_DYNAREC=ON -D BUILD_TYPE=beta -D USE_QT6=ON -D "EMU_BUILD=build ${pkgver}" -D "EMU_BUILD_NUM=$pkgver"
  cmake --build build
}
