...
build() {
  LDFLAGS='-z now -z shstk' cmake --preset regular -D CMAKE_INSTALL_PREFIX=/usr -D CMAKE_BUILD_TYPE=Release -D NEW_DYNAREC=ON -D "BUILD_TYPE=alpha"-D "EMU_BUILD=build ${pkgver}" -D "EMU_BUILD_NUM=$pkgver"
  cmake --build
}
